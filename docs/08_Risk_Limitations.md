...
- **Resolved 2026-08-31**: name/headline/location/photo (top-card) and about/experience/education/skills/certifications/languages (Flight-protocol sections) are all now confirmed live-verified against a real account. The three-day edge-level block on prior attempts (2026-08-29, 2026-08-30) turned out to be tied to an over-used session cookie, not the account — a fresh cookie from a normal browser session cleared it. Separately, the GraphQL query originally sourced for experience/education/skills/certifications had been retired by LinkedIn (`HTTP 500` from their own backend) in favor of a React Server Components rendering system; that parsing layer was rewritten against real captured responses (`src/extraction/implementation/linkedinFlightProtocol.ts`), including correctly attributing LinkedIn's multi-position (promotions-at-one-company) grouping to the right company rather than jumbling title/company/duration across entries.
- **Resolved 2026-08-31 (same day, second pass)**: a real user report against a densely-filled profile (19 positions, paragraph-length descriptions throughout) surfaced two further bugs in the same Flight-protocol layer. About and every experience/education description came back `null` — LinkedIn renders multi-paragraph text as a nested array of line segments rather than a single string, and the parser only handled the single-string case. Separately, the per-section preview response is itself paginated and was silently dropping entries past its first page (9 of 15 real positions came back). Both fixed: `linkedinFlightProtocol.ts` now reconstructs multi-line text from the nested segment shape, and experience additionally fetches LinkedIn's own "see all" details page for the complete entry list, sourcing descriptions from the (shorter but reliable) preview response matched by title+dates rather than by position — the details page renders full site chrome interleaved with the real list, so its own description positions aren't trustworthy. Education/skills/certifications/languages didn't hit the same pagination cap on the account tested and still come from the preview alone; the details-page fetch is the pattern to extend if a future profile does.
