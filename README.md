...
- **`linkedinExtractor.ts`** (tried first, when configured) — calls LinkedIn's internal Voyager **"Dash"** API (`voyagerIdentityDashProfiles`, `decorationId=...TopCardComplete-138`) with your session cookies, gated by a small self-imposed pacing module (`linkedinPacing.ts`: a minimum interval between calls plus a cooldown window after any detected block — LinkedIn's own blocking is account-level and behavior-based, not fingerprint-based, so disciplined pacing is the mitigation, not evasion tooling). Currently returns **top-card fields only** — name, headline, location, photo. Experience/education/skills/certifications/languages live behind separate, still-unverified GraphQL calls and were deliberately left out of this pass. This replaces an earlier version that called the classic `profileView` REST endpoint, which went from `410 Gone` to a hard block during this build (see below) — **this rewrite is implemented and unit-tested but not yet run against a live LinkedIn account**, so it's intentionally not deployed yet; until it's verified, `worker.ts` falls back to `publicExtractor` for every real request.
...
6. `npm run probe:linkedin -- <profile-url>` — diagnostic-only, local, not part of the request path. Fetches the top-card call plus the still-unverified experience/education/skills/certifications/languages/bio candidate endpoints (`scripts/probeLinkedinDashSections.ts`) and prints the raw JSON, so their real response shape can be captured once and turned into an accurate parser (see `.wayfinder` ticket T8) instead of a guessed one.
...
**Why `playwrightExtractor.ts` isn't deployed to Render**, independent of the block above: headless Chromium needs ~300–400MB RAM on top of the Node process, which risks OOM-crashing Render's free-tier instance (512MB total). It's kept as a local-only tool (`npm run extract:local -- <profile-url>`, needs `npx playwright install chromium` once) — proof of the approach and available to run against accounts/sessions that aren't currently blocked, without risking the live deployment.
...
- **LinkedIn's extraction surface is actively hostile right now, observed directly during this build** — see [Extraction layer](#extraction-layer) for the full account. Concretely: the classic Voyager `profileView` REST endpoint returns `410 Gone` (LinkedIn has moved to an internal "SDUI"/React-Server-Components protocol instead); LinkedIn is currently serving a signup/login wall to anonymous (logged-out) profile views as well, confirmed both through the API's `publicExtractor` and by checking a real logged-out browser directly; and even a headless-browser workaround using real session cookies (`playwrightExtractor.ts`, local-only) is blocked earlier, by an infinite redirect at LinkedIn's edge layer, reproducible with plain `curl`. The app handles all three failures correctly (a `422 PROFILE_PRIVATE_OR_UNREACHABLE`, never fabricated data), but no extraction path is guaranteed to return real data for a given profile/account at any given time — this is exactly the volatility risk called out below, not a bug in this codebase.
- **Experience/education/skills/certifications/languages are not populated yet** by either deployed extraction path — `publicExtractor` (anonymous JSON-LD) never had access to them, and `linkedinExtractor`'s Dash-based rewrite currently only covers the top-card fields (name/headline/location/photo); the remaining sections are unverified candidate endpoints, not shipped guesses (see [Extraction layer](#extraction-layer)). The response schema already supports these fields (omitted, not fabricated, when unavailable — same convention as everywhere else).
