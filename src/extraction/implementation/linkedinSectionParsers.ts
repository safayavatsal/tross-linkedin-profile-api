...
// that project's actual Python parsing code — see
// internal notes — not guessed. Still unverified
...
// a string), so unlike that project we don't need to parse it into date objects — keep the raw
// caption text as-is rather than porting that project's regex+dateutil date extraction, which
...
export function parseExperience(sectionJson: unknown): RawExperienceEntry[] {
  return sectionListElements(sectionJson).map((element): RawExperienceEntry => {
    ...
    // string field (that project splits it further only to populate its own separate emp_type
...
export function parseEducation(sectionJson: unknown): RawEducationEntry[] {
  return sectionListElements(sectionJson).map((element): RawEducationEntry => {
    ...
    return {
      ...
      // T11 Education #3: combined "Degree, Field of study" string, unsplit — that project doesn't
...
// T11 Languages: different queryId (see linkedinDashEndpoints.ts), name-only — that project never
...
// T11 Bio: that project uses fixed positional indices (elements[3].topComponents[1]) with no
