...
// request-building code (see internal notes and, for the per-section
// response field mapping, internal notes), not guessed.
//
// Endpoint shapes (queryId, variables) are sourced; response field mapping is sourced
// from T11's read of that project's parsers (src/extraction/implementation/linkedinSectionParsers.ts)
...
// (that project's actual per-section fetcher files): experience/education/skills/certifications
// all share one queryId; languages uses a *different* queryId on the same queryName/shape —
// confirmed from internal notes, not inferred.
